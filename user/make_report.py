...
def make_report(message:Message, chat_id:str, username:str, partner_id:str):
    ...
    bot.reply_to(message, "Berhasil dilaporkan")
    if chat_id in users.split():
        pass
    else:
        bot.send_message(partner_id, "Anda telah dilaporkan oleh %s, jumlah laporan yang tersisa hingga Anda dilarang %s" % (username, (max_reports) - (user_reports+1)))
        users += " %s" % chat_id
